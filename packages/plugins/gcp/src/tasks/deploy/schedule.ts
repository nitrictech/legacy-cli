...
/**
 * Create a Cloud Scheduler Job to act as a Nitric Schedule in GCP
 * @param schedule to deploy
 * @param topics to publish scheduled events to
 */
export function createSchedule(schedule: NamedObject<NitricSchedule>, topics: DeployedTopic[]): DeployedSchedule {
	...
	throw new Error(`misconfiguration error ${schedule.target.id} does not exist in stack`);
}
