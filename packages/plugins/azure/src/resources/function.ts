...
interface NitricFunctionAzureAppServiceArgs {
	/**
...
/**
 * Azure App Service implementation of a Nitric Function
 */
export class NitricFunctionAzureAppService extends pulumi.ComponentResource {
	public readonly name: string;
	public readonly webapp: web.WebApp;

	constructor(name: string, args: NitricFunctionAzureAppServiceArgs, opts?: pulumi.ComponentResourceOptions) {
		super('nitric:func:AppService', name, {}, opts);
		const defaultResourceOptions: pulumi.ResourceOptions = { parent: this };
		const { func, resourceGroup, plan, registry, image, topics } = args;
		...
		this.webapp = new web.WebApp(
			func.getName(),
			{
				...
				name: `${func.getStack().getName()}-${func.getName()}`,
		...
		(triggers.topics || []).forEach((s) => {
			...
			if (topic) {
				new eventgrid.EventSubscription(
					`${func.getName()}-${topic.name}-subscription`,
					{
						eventSubscriptionName: `${func.getName()}-${topic.name}-subscription`,
