...
export class NitricServiceAWSLambda extends pulumi.ComponentResource {
	...
	constructor(name, args: NitricServiceAwsLambdaArgs, opts?: pulumi.ComponentResourceOptions) {
		...
		const lambdaRole = new aws.iam.Role(`${service.getName()}LambdaRole`, {
			assumeRolePolicy: aws.iam.assumeRolePolicyForPrincipal(aws.iam.Principals.LambdaPrincipal),
		}, defaultResourceOptions);
	
		new aws.iam.RolePolicyAttachment(`${service.getName()}LambdaBasicExecution`, {
			policyArn: aws.iam.ManagedPolicies.AWSLambdaBasicExecutionRole,
			role: lambdaRole.id,
		}, defaultResourceOptions);
	
		// TODO: Lock this SNS topics for which this function has pub definitions
		new aws.iam.RolePolicy(`${service.getName()}SNSAccess`, {
			role: lambdaRole.id,
		...
		new aws.iam.RolePolicy(`${service.getName()}DynamoDBAccess`, {
			role: lambdaRole.id,
			policy: JSON.stringify({
				Version: '2012-10-17',
				Statement: [
		...
		this.lambda = new aws.lambda.Function(service.getName(), {
		...
		(triggers.topics || []).forEach((triggerTopic) => {
			...
			if (topic) {
				new aws.lambda.Permission(`${service.getName()}${triggerTopic}Permission`, {
				...
				new aws.sns.TopicSubscription(`${service.getName()}${triggerTopic}Subscription`, {
